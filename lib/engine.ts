...
export class GameEngine {
  ...
  applyServerImpact(event: {
    ...
    y: number;
    radius: number;
    state: Parameters<GameEngine['applyServerState']>[0];
  }) {
  ...
  explode(cx: number, cy: number, radius: number, damage: number, type: string) {
    // Terrain destruction
