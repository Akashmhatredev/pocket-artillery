export class GameEngine {
  ...
  // Spawn my own shot the instant FIRE is pressed instead of waiting a full
  // server round-trip. Uses the exact same launch math as the server
  // (simulate_projectile in stdb-module), so when the authoritative spawn
  // event arrives it can silently adopt this projectile mid-flight.
  spawnOptimisticProjectile(weaponId, angle, power) {
    if (this.isFiring || this.winner) return;
    const p = this.players[this.currentPlayerIndex];
    if (!p) return;
    const rad = (angle * Math.PI) / 180;
    const speed = power * 0.35 + 2;
    ...
    this.projectiles.push({
      ...
      type: weaponId,
      authoritative: true, // server resolves the impact; no local collision
    });
    this.isFiring = true;
    this.notifyUI();
  }

  // The server rejected the shot (or it never happened) — remove the preview.
  cancelOptimisticProjectile() {
    const before = this.projectiles.length;
    this.projectiles = this.projectiles.filter((projectile) => projectile.id !== 'optimistic');
    if (this.projectiles.length !== before) {
      this.isFiring = this.projectiles.length > 0;
      this.notifyUI();
    }
  }

  spawnNetworkProjectile(event, isMine = false) {
    // My own shot already flies as the optimistic preview; adopt the server's
    // id so the impact event can find it, and keep its advanced position
    // (the event's start coords are a round-trip old).
    if (isMine) {
      const optimistic = this.projectiles.find((projectile) => projectile.id === 'optimistic');
      if (optimistic) {
